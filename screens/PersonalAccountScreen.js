...
export default function PersonalAccountScreen({ navigation, route }) {
	...
	useEffect(() => {
		setUserData(route.params?.userData ?? getUser());
			...
			>
				<ScrollView width="100%">
					...
					>
						...
						{userData.email ? (
							<H3 style={{ marginVertical: 7 }}>{userData.email}</H3>
						) : null}
						{userData.phone ? (
							<H3 style={{ marginVertical: 7 }}>{userData.phone}</H3>
						) : null}
						{userData.birthday ? (
							<H3 style={{ marginVertical: 7 }}>
								{formatDate(
						...
						{userData.address ? (
							<>
								...
								/>
								<H3 style={{ marginVertical: 7 }}>{userData.address}</H3>
							</>
						) : null}
					</LinearGradient>
					{/*
